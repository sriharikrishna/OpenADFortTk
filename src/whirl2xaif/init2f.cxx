// $Header: /Volumes/cvsrep/developer/OpenADFortTk/src/whirl2xaif/Attic/init2f.cxx,v 1.2 2003/05/14 01:10:12 eraxxon Exp $
// -*-C++-*-
...
//
// File:
//   $Source: /Volumes/cvsrep/developer/OpenADFortTk/src/whirl2xaif/Attic/init2f.cxx,v $
//
// Purpose:
...
//
// Based on Open64 be/whirl2f/init2f.cxx
//
//***************************************************************************
			   ...
			   UINT         tmpvar_idx)
{
    ...
    * memory reference.
    */
//   Prepend_F77_Indented_Newline(xos, 1, NULL/*label*/);
#if 0//REMOVE
   Prepend_Token_Special(xos, ')');
   Prepend_Token_String(xos, W2CF_Symtab_Nameof_Tempvar(tmpvar_idx));
   Prepend_Token_Special(xos, ',');
   Prepend_And_Copy_Token_List(xos, name1_tokens);
   Prepend_Token_Special(xos, '(');
   Prepend_Token_String(xos, "EQUIVALENCE");
   Prepend_F77_Indented_Newline(xos, 1, NULL/*label*/);
#endif //FIXMEprepend
			  ...
			  INT           repeat)
{
   ...
   if (repeat > 1)
   {
      ...
      Prepend_Token_String(*init_tokens, Num2Str(repeat, "%llu"));
#endif
   }
#if 0//FIXME
   if (!Is_Empty_Token_Buffer(xos))
      xos << ",";

   Append_And_Reclaim_Token_List(xos, init_tokens);
#endif
	      ...
	      TY_IDX       object_ty)
{
    ...
    */
   xml::ostream val_tokens = New_Token_Buffer();
		    ...
		    UINT         repeat)
{
  ...
  switch (INITV_kind(initv))
   ...
   case INITVKIND_VAL:
      INITVKIND_val(xos, repeat, &Tcon_Table[INITV_tc(initv)], object_ty);
      break;
      
   case INITVKIND_ONE:
      INITVKIND_const(xos, repeat, one_consts, object_ty);
      break;

   case INITVKIND_ZERO:
      INITVKIND_const(xos, repeat, zero_consts, object_ty);
      break;
			  ...
			  XlationContext  context)
{
   ...
   {
      xos << "(";
      Append_Token_String(xos, Num2Str(string_ofst+1, "%llu"));
      xos << ":";
      Append_Token_String(xos, Num2Str(string_ofst+string_size, "%llu"));
      xos << ")";
   }
			     ...
			     const ARRAY_SEGMENT *aseg)
{
   ...
   for (initv_idx = aseg->first_idx; initv_idx <= aseg->last_idx; initv_idx++)
   {
	 ...
	 repeat = INIT2F_choose_repeat(Initv_Table[initv]);
      ...
      INITVKIND_translate(xos, initv, aseg->etype, repeat);
   } /* for */
...
# if 0//June
   ...
   Append_Token_String(xos, 
	    Num2Str(aseg->start_ofst/TY_size(aseg->etype) + 1,
			     "%llu"));
# endif

   /********************************************************************/
   /* Maybe think about chang more for DATA fzhao----June              */
   /*here only suppose array in DATA is always one-dimension,and       */
   /* initialization is for whole array                                */
   /********************************************************************/
   TCON2F_translate(xos, Host_To_Targ(MTYPE_I4, ARB_lbnd_val(arb)),
		    FALSE /*is_logical*/);
...
// June#if 0
   Append_Token_String(xos, 
		       Num2Str(aseg->end_ofst/TY_size(aseg->etype)+ 
                                                           ARB_lbnd_val(arb)-1,
					"%llu"));
//#endif
# if 0

     TCON2F_translate(xos, Host_To_Targ(MTYPE_I4, ARB_ubnd_val(arb)),
		      FALSE /*is_logical*/);
			   ...
			   const ARRAY_SEGMENT *aseg)
{
   ...
   XlationContext      context;
   xml::ostream&      abase_tokens, aref_tokens;
	     ...
	     (aseg->end_ofst - aseg->start_ofst)/TY_size(aseg->etype)))
   {
	 ...
	 INIT2F_Translate_Char_Ref(aref_tokens,
				   ...
				   context);
	 INIT2F_Append_Initializer(xos, &aref_tokens, 1);
   ...
   else /* Each initv corresponds to exactly one array element */
   {
	 ...
	 INIT2F_Append_Initializer(xos, &abase_tokens, 1);
      }
