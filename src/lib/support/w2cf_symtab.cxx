...
//
// File:
//   $Source: /Volumes/cvsrep/developer/OpenADFortTk/src/lib/support/Attic/w2cf_symtab.cxx,v $
//
// Purpose:
...
//
// Based on Open64 be/whirl2c/w2cf_symtab.cxx
//
//***************************************************************************
...
W2CF_Symtab_Nameof_St_Pointee(const ST *st)
{
   ...
   * name appended to the suffix (hence the "+32").
   */
  pointee_name = StrCat("deref_", W2CF_Symtab_Nameof_St(st));
...
W2CF_Symtab_Nameof_Fld_Pointee(FLD_HANDLE fld)
{
    ...
    * name appended to the suffix (hence the "+32").
    */
   pointee_name = StrCat("deref_", W2CF_Symtab_Nameof_Fld(fld));
   symname = Get_Name_Buf_Slot(strlen(pointee_name) + 32);
   W2CF_Get_Basename(pointee_name, symname, &symid);
